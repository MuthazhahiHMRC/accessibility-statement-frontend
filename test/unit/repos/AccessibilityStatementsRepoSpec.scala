...
import uk.gov.hmrc.accessibilitystatementfrontend.models.{AccessibilityStatement, Archived, Draft, FullCompliance, Public}
...
class AccessibilityStatementsRepoSpec
  ...
  def buildAppConfig(showDraftStatementsEnabled: Boolean, showArchivedStatementsEnabled: Boolean) = {
    ...
    when(
      appConfig.showArchivedStatementsEnabled
    ) thenReturn showArchivedStatementsEnabled
    appConfig
  }

  private val appConfig    = buildAppConfig(showDraftStatementsEnabled = false, showArchivedStatementsEnabled = false)
  ...
  "findByServiceKeyAndLanguage" should {
    ...
    "find a draft service if feature show draft toggle is enabled" in {
      val appConfigWithDraftsEnabled =
        buildAppConfig(showDraftStatementsEnabled = true, showArchivedStatementsEnabled = false)
      val repo                       =
    ...
    "find an archived service if feature show archived toggle is enabled" in {
      val appConfigWithDraftsEnabled =
        buildAppConfig(showDraftStatementsEnabled = false, showArchivedStatementsEnabled = true)
      val repo                       =
        AccessibilityStatementsSourceRepo(
          appConfigWithDraftsEnabled,
          servicesFinder,
